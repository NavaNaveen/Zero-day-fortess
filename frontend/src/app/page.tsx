...
export default function Dashboard() {
  ...
  const hasTimings = !!(session && Object.keys(session.agent_timings).length > 0);
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        {mode === "single" && (
          <>
                  ...
                  <div className="mt-4">
                    {singleActive ? (
                      <MatrixRain
                        height={220}
                        label={
                          session?.state === "recon" ? "RECON IN PROGRESS..."
                          : session?.state === "assault" ? "EXPLOITING TARGETS..."
                          : session?.state === "chaining" ? "BUILDING ATTACK CHAINS..."
                          : session?.state === "patching" ? "PATCHING VULNERABILITIES..."
                          : session?.state === "hardening" ? "HARDENING DEFENSES..."
                          : session?.state === "reporting" ? "GENERATING REPORT..."
                          : "SCANNING..."
                        }
                      />
                    ) : (
                ...
                <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1">
                  {session?.vulnerabilities.length === 0 && (
                    <div className="text-slate-600 text-xs text-center py-8">
                      {singleActive ? "Scanning..." : "Start a battle to begin scanning..."}
                    </div>
                  )}
            ...
            {/* 4. Bottom section — single gap-4 child, conditional content inside */}

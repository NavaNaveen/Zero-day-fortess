...
async def run_battle(session: BattleSession) -> BattleSession:
    ...
    """
    await broadcast("battle_started", {
    ...
    try:
        ...
        hardening_result = await _timed(session, "Fortress", hardening.run(session))
        if isinstance(hardening_result, dict) and hardening_result:
            session.hardening_actions = hardening_result.get("hardening_actions", [])
            session.security_score_improvement = hardening_result.get("security_score_improvement", 0)
        battle_store.save(session)
